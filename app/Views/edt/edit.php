...
<div class="container py-4" style="max-width:780px">
  ...
  <div class="d-flex align-items-center mb-4 gap-3">
    <a href="/edt/semaine/<?= esc($creneau['semaine']) ?>" class="btn btn-outline-secondary btn-sm">
      <i class="bi bi-arrow-left"></i>
    </a>
    <h4 class="mb-0"><i class="bi bi-pencil-square me-2"></i>Modifier le créneau</h4>
  </div>
  ...
  <div class="card shadow-sm">
    <div class="card-body">
      <form id="formEdt" action="/edt/update/<?= $creneau['id'] ?>" method="POST">
        <?= csrf_field() ?>
        ...
        <div class="mb-3">
          ...
          <input type="date" name="semaine" class="form-control"
                 value="<?= old('semaine', $creneau['semaine']) ?>" required>
        </div>
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <select name="filiere_id" id="filiere_id" class="form-select" required>
              ...
              <?php foreach ($filieres as $f): ?>
                <option value="<?= $f['id'] ?>"
                  <?= old('filiere_id', $creneau['filiere_id']) == $f['id'] ? 'selected' : '' ?>>
                  <?= esc($f['nom']) ?>
                </option>
              <?php endforeach; ?>
            </select>
            ...
            <select name="cours_id" id="cours_id" class="form-select" required>
              <?php foreach ($cours as $c): ?>
                <option value="<?= $c['id'] ?>" data-filiere="<?= $c['filiere_id'] ?>"
                  <?= old('cours_id', $creneau['cours_id']) == $c['id'] ? 'selected' : '' ?>>
                  <?= esc($c['intitule']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-6">
            ...
            <select name="enseignant_id" id="enseignant_id" class="form-select" required>
              <?php foreach ($enseignants as $e): ?>
                <option value="<?= $e['id'] ?>"
                  <?= old('enseignant_id', $creneau['enseignant_id']) == $e['id'] ? 'selected' : '' ?>>
                  <?= esc($e['prenom'] . ' ' . $e['nom']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-6">
            ...
            <select name="salle_id" id="salle_id" class="form-select" required>
              <?php foreach ($salles as $s): ?>
                <option value="<?= $s['id'] ?>"
                  <?= old('salle_id', $creneau['salle_id']) == $s['id'] ? 'selected' : '' ?>>
          ...
          <div class="col-md-4">
            ...
            <select name="jour" id="jour" class="form-select" required>
              <?php foreach ($jours as $j): ?>
                <option value="<?= $j ?>"
                  <?= old('jour', $creneau['jour']) == $j ? 'selected' : '' ?>>
                  <?= $j ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-4">
            ...
            <select name="heure_debut" id="heure_debut" class="form-select" required>
              <?php foreach ($creneaux as $hd => $hf): ?>
                <option value="<?= $hd ?>"
                  <?= old('heure_debut', substr($creneau['heure_debut'],0,5)) === $hd ? 'selected' : '' ?>>
                  <?= $hd ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-4">
            ...
            <select name="heure_fin" id="heure_fin" class="form-select" required>
              <?php foreach ($creneaux as $hd => $hf): ?>
                <option value="<?= $hf ?>"
                  <?= old('heure_fin', substr($creneau['heure_fin'],0,5)) === $hf ? 'selected' : '' ?>>
                  <?= $hf ?>
                </option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="d-flex gap-2 mt-4">
          ...
          <a href="/edt/semaine/<?= esc($creneau['semaine']) ?>" class="btn btn-outline-secondary">
            Annuler
          </a>
          <button type="button" id="btnVerifier" class="btn btn-outline-warning ms-auto">
...
document.getElementById('filiere_id').addEventListener('change', function () {
    ...
    Array.from(document.getElementById('cours_id').options).forEach(opt => {
...
document.getElementById('heure_debut').addEventListener('change', function () {
    ...
    if (fin) {
        Array.from(document.getElementById('heure_fin').options).forEach(opt => {
...
async function verifierConflits() {
    const body = new URLSearchParams(collectData());
...
document.getElementById('formEdt').addEventListener('submit', async function (e) {
    e.preventDefault();
    if (await verifierConflits() === 0) this.submit();
});
</script>
