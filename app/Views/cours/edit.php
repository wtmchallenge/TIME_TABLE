...
.form-card{max-width:580px;margin:0 auto;background:white;border-radius:16px;box-shadow:0 4px 24px rgba(26,35,126,.1);overflow:hidden;}
...
<div class="mb-3 d-flex align-items-center gap-2">
    ...
    <span style="color:#9e9e9e;font-size:.85rem">/ Modifier cours</span>
</div>

<div class="form-card">
    <div class="form-card-header">
        <h4>✏️ Modifier le cours</h4>
        <p>ID #<?= $cours['id'] ?> — <?= esc($cours['intitule']) ?></p>
    </div>
    <div class="form-card-body">
        ...
        <?php endif; ?>
        <form action="/cours/update/<?= $cours['id'] ?>" method="POST">
            <?= csrf_field() ?>
            <div class="mb-3">
                <label class="lbl">Intitulé du cours</label>
                <input type="text" name="intitule" class="inp" value="<?= old('intitule', $cours['intitule']) ?>">
            </div>
            <div class="mb-3">
                ...
                <select name="filiere_id" class="sel">
                    <?php foreach($filieres as $f): ?>
                        <option value="<?= $f['id'] ?>"
                            <?= (old('filiere_id', $cours['filiere_id']) == $f['id']) ? 'selected' : '' ?>>
                            <?= esc($f['nom']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="volume_horaire" class="inp" value="<?= old('volume_horaire', $cours['volume_horaire']) ?>" min="1">
            </div>
